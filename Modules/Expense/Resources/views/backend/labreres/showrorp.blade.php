...
@section('content')
    ...
    <div class="container ">
        <center>
            @if($type=='r')
                <h1 class="text-success mt-4">{{__('reservation.showReward')}}</h1>
            @else
                <h1 class="text-success mt-4">{{__('reservation.showPunishment')}}</h1>
        ...
        <div clsss="table-responsive">
            ...
            <div class="row">
                <div class="float-right col col-md-8">
                    <a href="{{ $type == 'r' ? route('frontend.users.createReward') : route('frontend.users.createPunishment') }}" class="btn btn-success  " title="Create Items">
                        <i class="fas fa-plus-circle"></i>

                    </a>
                    <label class="font-weight-bold"> {{ $type == 'r' ?__('reservation.addReward') :  __('reservation.addPunishment')}}</label>
                </div>
            </div>
            <br>
            {{-- testcode --}}
            <table id="data-table" class="table table-bordered data-table table-condensed table-hover">
                <thead>
                <tr>
                    <th class=' '>{{__('reservation.laborere')}}</th>
                    <th class=' '>{{__('reservation.value')}}</th>
                    <th class=' '>{{__('reservation.reason')}}</th>
                    <th class=' '>{{__('reservation.date')}}</th>
    ...
    <script type="text/javascript">
        $(document).ready(function populate() {
            ...
            var table = $('.data-table').DataTable({
                ...
                @if($type=='r')
                ajax: "{{ route('frontend.users.RewardOrPunishment',$type) }}" ,
                @else
                ajax: "{{ route('frontend.users.RewardOrPunishment',$type) }}",
            ...
            $(document).on('click', '#ok', function (e) {
                ...
                $.ajax({
                    url: "{{ route('frontend.users.RewardorPunishmentDelete') }}/"+ id  ,
                    type: 'get',
                    dataType: 'json',
